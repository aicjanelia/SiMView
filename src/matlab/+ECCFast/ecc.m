function [results, warp, warpedImage] = ecc(image, template, levels, noi, transform, delta_p_init,learningRate, ptsPerIter)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% DGA update
final_warp_for_imwarp = final_warp; %imwarp does negative translation/angle compared to spatial_interp
final_warp_for_imwarp(1,[2,3])=-final_warp(1,[2,3]);
final_warp_for_imwarp(2,[1,3])=-final_warp(2,[1,3]);
warpedImage = imwarp(initImage, affine2d(final_warp_for_imwarp'), 'outputview', imref2d(size(initImage)),'interp','cubic');

%for ii = 1:sZi3
%   warpedImage(:,:,ii) = spatial_interp(double(initImage(:,:,ii)), final_warp, transform, nx2, ny2);
%end

%warpedImage = uint8(warpedImage);
warp = affine2d(final_warp_for_imwarp');
%%%